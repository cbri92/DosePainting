...
def convert_nii_to_dicom_RTdosefile(nii_image, dcm_dir, output_directory=".", out_filename="dose.dcm"):
    """Converts a NII image to a Dicom RT dose file using a DICOM series of reference
    Args:
    ...
    # ds.ImageOrientationPatient = list(nii_image.GetDirection()[0:6])
    ...
    # ds.SamplesPerPixel = nii_image.GetNumberOfComponentsPerPixel()
    ...
    ds.Rows = int(nii_image.GetSize()[1])
    ds.Columns = int(nii_image.GetSize()[0])
    ds.PixelSpacing = list(nii_image.GetSpacing()[0:2])
    ...
    slice_thickness = nii_image.GetSpacing()[2]
